...
// AddCapabilities adds capabilities to the host configuration for the container.
// parameters:
//   - caps: the capabilities to add
func (h *hostSetters) AddedCapabilities(caps ...Capability) config.SetHostConfig {
...
// DropCapabilities drops capabilities from the host configuration for the container.
// parameters:
//   - caps: the capabilities to drop
func (h *hostSetters) DroppedCapabilities(caps ...Capability) config.SetHostConfig {
	return func(opt *container.HostConfig) error {
		for _, cap := range caps {
...
func (h *hostSetters) DroppedAllCapabilities() config.SetHostConfig {
	return h.DroppedCapabilities(ALL)
}

// DropSensitiveCapabilities drops a common set of sensitive capabilities
//
// SYS_ADMIN, SYS_MODULE, SYS_PTRACE, SYS_TIME, SYSLOG, MAC_ADMIN, MAC_OVERRIDE, CHECKPOINT_RESTORE
func (h *hostSetters) DroppedSensitiveCapabilities() config.SetHostConfig {
	return h.DroppedCapabilities(
